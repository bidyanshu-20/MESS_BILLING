...
const ChatWithAdmin = () => {
    ...
    const userId = selectedUser?._id || selectedUser?.id || currentUser?._id;
    const userName = selectedUser?.name || currentUser?.name || "User";
    ...
    useEffect(() => {
        fetch(`${API_BASE}/api/chat/admin`)
            ...
            .catch((error) => {
                console.log("Failed to load admin:", error);
    ...
    useEffect(() => {
        if (!userId || !admin?._id) {
            if (admin?._id) setError("Chat user details are missing. Please login again.");
            return;
        }
        ...
        socket.current.on("receiveMessage", (data) => setMessages((prev) => [...prev, data]));
    ...
    const sendMessage = () => {
        if ((!message.trim() && !selectedFile) || !socket.current || !admin?._id || !userId) return;
    ...
    const deleteMessage = (messageId, deleteType) => {
        ...
        socket.current.emit(
            "deleteMessage",
            { messageId, currentUserId: userId, roomId: userId, deleteType },
            (response) => {
                if (!response?.success) {
    ...
    const contactName = admin?.name || "Admin";
    return (
        <div className="min-h-screen bg-slate-900 text-white flex flex-col">
            <div className="bg-slate-800 px-6 py-4 border-b border-slate-700 flex items-center gap-3">
                <div className="h-11 w-11 rounded-full bg-blue-600 flex items-center justify-center font-bold overflow-hidden">
                    {admin?.profilePic ? (
                        <img src={admin.profilePic} alt={contactName} className="h-full w-full object-cover" />
                    ) : (
                ...
                <div>
                    ...
                    <p className="text-slate-400 text-sm">Signed in as {userName}</p>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
                {messages.map((msg, index) => {
                    ...
                    const senderName = isMine ? userName : contactName;
                    return (
                        <div key={msg._id || index} className={`flex gap-2 ${isMine ? "justify-end" : "justify-start"}`}>
                            ...
                            <div className={`max-w-[72%] flex flex-col ${isMine ? "items-end" : "items-start"}`}>
                                ...
                                <div className={`relative group px-4 py-2 rounded-2xl break-words ${isMine ? "bg-blue-600" : "bg-slate-700"} ${msg.deletedForEveryone ? "italic text-slate-200" : ""}`}>
                                    {msg._id && (
                                        <button
            ...
            <div className="bg-slate-800 border-t border-slate-700 p-4 flex gap-3">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="bg-slate-700 hover:bg-slate-600 px-4 py-3 rounded-xl font-medium">
                    Attach
                </button>
                ...
                <input
                    ...
                    className="flex-1 bg-slate-700 px-4 py-3 rounded-xl outline-none"
                />

                <button onClick={sendMessage} className="bg-blue-600 hover:bg-blue-700 px-6 py-3 rounded-xl font-medium">
                    Send
                </button>
            </div>
        </div>
